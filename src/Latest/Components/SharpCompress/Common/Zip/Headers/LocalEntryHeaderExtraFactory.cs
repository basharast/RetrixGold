...
namespace SharpCompress.Common.Zip.Headers
{
    ...
    internal static class LocalEntryHeaderExtraFactory
    {
        internal static ExtraData Create(ExtraDataType type, ushort length, byte[] extraData)
        {
            switch (type)
            {
                case ExtraDataType.UnicodePathExtraField: return new ExtraUnicodePathExtraField(type, length, extraData);
                case ExtraDataType.Zip64ExtendedInformationExtraField: return new Zip64ExtendedInformationExtraField(type, length, extraData);
                default: return new ExtraData(type, length, extraData);
            }
        }
    }
}
