...
namespace LibRetriX.RetroBindings.Tools
{
    ...
    {
        private const int LibretroDeviceTypeShift = 8;

        public static uint GenerateDeviceSubclass(uint baseId, uint id)
        {
            var output = ((id + 1) << LibretroDeviceTypeShift) | baseId;
            return output;
        }
                ...
                case Constants.RETRO_DEVICE_KEYBOARD:
                    {
                        if (id > 0)
                        {
                            ...
                            {
                                if (((uint)kItem.Value).Equals(id))
                                { 
                                    return (InputTypes)kItem.Key;
                                }
                            }
                        }
                        return InputTypes.DeviceIdUnknown;
