...
namespace Visualizer.UI.Spectrum
{
    public class LineSpectrum : SpectrumBase
    {
        ...
        public LineSpectrum(FftSize fftSize, GamePlayerViewModel viewModel)
        {
            FftSize = fftSize;
            VM = viewModel;
        }
        ...
        private void CreateSpectrumLineInternal(ICanvasAnimatedControl canvas, CanvasDrawingSession ds, float[] fftBuffer, Size size)
        {
            try
            {
                ...
                try
                {
                    c1.A = (byte)Math.Round((255 * (GamePlayerView.BarOpacity / 100.0)));
                    c2.A = (byte)Math.Round((255 * (GamePlayerView.BarOpacity / 100.0)));
                }
                catch (Exception ex)
                {
                    c1.A = 200;
                ...
                using (var brush = new CanvasLinearGradientBrush(canvas, c1, c2))
                {
                    ...
                    for (int i = 0; i < spectrumPoints.Length; i++)
                    {
                        ...
                        var point0 = new Vector2(xCoord + (GamePlayerView.AutoScaleDown ? 10: 0), height);
                        var point1 = new Vector2(xCoord + (GamePlayerView.AutoScaleDown ? 10 : 0), height - (float)p.Value);
        ...
        public override void UpdateFrequencyMapping()
        {
            try
            {
                _barWidth = Math.Max(((_currentSize.Width / (GamePlayerView.AutoScaleDown ? (BarCount == 1 ? 12 : (BarCount == 2 ? 8 : 6)) : 1) - (BarSpacing * (BarCount + 1))) / BarCount), 0.00001);
