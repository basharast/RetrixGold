...
namespace WinUniversalTool.Views
{
    ...
    public sealed partial class ContentDialogCustom : ContentDialog, IDisposable
    {
        ...
        public ContentDialogCustom(string title, string message, string button1, string button2, string button3, bool telnetOutput = false, string button1Temp = null, string button2Temp = null, string button3Temp = null)
        {
            ...
            if (PlatformService.UseColoredDialog)
            {
                try
                {
                    try
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    if (itemSplit != null && itemSplit.Length > 1)
                                    {
                                        ...
                                        {
                                            forCounter++;
                                            var testIfPath = Helpers.isLinkFile(itemSplitItem);
                                            if (testIfPath)
                                            {
                                                string Name = "";
                                                try
                                            ...
                                            else
                                            {
                                                if (itemSplitItem.Length > 0)
                                                {
                                                    if (forCounter == 1)
                                                    {
                                                        ...
                                                        else if (itemSplitItem.ToLower().Contains("advice"))
                                                        {
                                                            messageText.Inlines.Add(new Run { Text = $"{itemSplitItem}:", FontWeight = FontWeights.Bold, Foreground = new SolidColorBrush(Colors.Green) });
                                                        }
                                    ...
                                    {
                                        var testIfPath = Helpers.isLinkFile(textSplitItemTemp);
                                        if (testIfPath)
                                        {
                                            string Name = "";
                                            try
                    ...
                    else
                    {
                        try
                        {
                            var testIfPath = Helpers.isLinkFile(message);
                            if (testIfPath)
                            {
                                string Name = "";
                                try
        ...
        private void ButtonColor(Button btn)
        {
            ...
            try
            {
                try
                {
                    ...
                    if (btn.Tag.ToString().ToLower().Contains("reset"))
                    {
                        btn.Background = new SolidColorBrush(Colors.OrangeRed);
                    }
                    else
        ...
        public ContentDialogCustom(string title, Run[] message, string button1, string button2, string button3, bool telnetOutput = false, string button1Temp = null, string button2Temp = null, string button3Temp = null)
        {
            ...
            foreach (var runItem in message)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        var testIfPath = Helpers.isLinkFile(runItem.Text);
                        if (testIfPath)
                        {
                            var Name = "";
                            try
            ...
            if (Helpers.DialogInProgress)
            {
                try
                {
                    ...
                    if (totalWait > 7)
                    {
                        ...
                        PlatformService.ShowNotification(localMessage, 5);
                    }
                    else
                    {
    ...
    public class DialogsSettings : INotifyPropertyChanged
    {
        private double _dialogMinSize = 100;
        public double DialogMinSize
        {
            get { return _dialogMinSize; }
            set { _dialogMinSize = value; OnPropertyChanged(); }
        }
