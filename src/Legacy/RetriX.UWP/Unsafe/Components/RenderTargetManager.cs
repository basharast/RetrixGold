...
namespace RetriX.UWP.Components
{
    internal class RenderTargetManager : IDisposable
    {
        ...
        public void Render(CanvasDrawingSession drawingSession, ICanvasAnimatedControl canvas)
        {
            try
            {
                ...
                lock (RenderTargetLock)
                {
                    ...
                    try
                    {
                        if (GamePlayerView._lineSpectrum != null)
                        {
                            try
                            {
                                GamePlayerView._lineSpectrum.CreateSpectrumLine(canvas, drawingSession);
                            }
                            catch (Exception ex)
                            {
        ...
        public ICanvasImage UpdateEffects(CanvasDrawingSession drawingSession, CanvasBitmap RenderTarget, double viewportWidth, double viewportHeight, CanvasImageInterpolation interpolation)
        {
            ...
            try
            {
                ...
                BlendEffectMode blendEffectMode = BlendEffectMode.Overlay;
                bool blendModeState = false;
                    ...
                    try
                    {
                        switch (effect.Name)
                        {
                            ...
                            case "OverlayEffect":
                                foreach (var eValue in effect.Values1)
                                {
                                    if (effect.tempResult == null || effect.Updated)
                                    {
                                    ...
                                    blendModeState = effect.BlendModeState;
                                    if (blendModeState)
                                    {
                                        blendEffectMode = effect.CurrentBlendMode;
                                    }
                                }
                                break;
                    ...
                    foreach (var overlayItem in outputOverlays)
                    {
                        if (PlatformService.FitOverlay)
                        {
                            ...
                            if (blendModeState)
                            {
                                ...
                                blendEffect.Mode = blendEffectMode;
                                blendEffect.CacheOutput = true;
                                drawingSession.DrawImage(blendEffect, new Rect(-0.5, -0.5, 1, 1), RenderTargetViewport, 1.0f, interpolation);
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            {
                                drawingSession.DrawImage(overlayItem, new Rect(-0.5, -0.5, 1, 1), RenderTargetViewport, 0.9f, interpolation);
                            }
                        }
                    }
                }
        ...
        int trueWidth = 0;
        int trueHeight = 0;
        public unsafe void UpdateFromCoreOutput(CanvasAnimatedControl renderPanel, IntPtr data, uint width, uint height, uint pitch)
        {
            var device = renderPanel.Device;
            try
            {
                if (data == IntPtr.Zero || RenderTarget == null || PlayAudioOnly)
                    return;
                ...
                lock (RenderTargetLock)
                {
                    ...
                    */
                    ...
                    using (var renderTargetMap = new BitmapMap(device, RenderTarget))
                    {
                        ...
                        var mapData = new IntPtr(renderTargetMap.Data);
                        var length = (int)RenderTarget.Size.Height * mapPitch;

                        if (mapData == IntPtr.Zero)
                        {
                            return;
                        }
                        switch (CurrentPixelFormat)
                        {
                            case PixelFormats.RGB0555:
                                FramebufferConverter.ConvertFrameBufferRGB0555ToXRGB8888(width, height, data, inputPitch, mapData, length, mapPitch);
                                break;
                            case PixelFormats.RGB565:
                                FramebufferConverter.ConvertFrameBufferRGB565ToXRGB8888(width, height, data, inputPitch, mapData, length, mapPitch);
                                break;
                            case PixelFormats.XRGB8888:
                                FramebufferConverter.ConvertFrameBufferToXRGB8888(width, height, data, inputPitch, mapData, length, mapPitch);
                                break;
                        }
                    }
                }
